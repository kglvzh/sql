...
## Задание 1. Создание первой базы данных (представлена по умолчанию) и таблицы
...
Вот ответы на контрольные вопросы, оформленные **одним предложением на каждый вопрос**:

**LowCardinality(String) эффективнее обычного String для столбца category**, потому что при малом количестве уникальных значений этот тип автоматически создаёт словарь и хранит не сами строки, а их числовые индексы, что значительно сокращает объём занимаемой памяти и ускоряет выполнение запросов с GROUP BY и фильтрацией.

2. В чём разница между ORDER BY и PRIMARY KEY в ClickHouse?
**Разница между ORDER BY и PRIMARY KEY в ClickHouse** заключается в том, что ORDER BY определяет физический порядок сортировки данных на диске и является обязательным параметром для движков семейства MergeTree, а PRIMARY KEY задаёт только разреженный индекс для ускорения поиска и может быть подмножеством ORDER BY, но не гарантирует уникальность записей.

3. Когда следует использовать ReplacingMergeTree вместо MergeTree?
**ReplacingMergeTree следует использовать вместо MergeTree**, когда нужно хранить только последнюю версию строки для каждого ключа сортировки (например, в справочниках или профилях пользователей), а старые версии данных должны автоматически удаляться при фоновом слиянии партиций.

4. Почему SummingMergeTree не заменяет GROUP BY в аналитических запросах?
**SummingMergeTree не заменяет GROUP BY в аналитических запросах**, потому что суммирование числовых столбцов в этом движке происходит асинхронно и только при фоновом слиянии кусков данных, поэтому без явного GROUP BY запрос может вернуть частично просуммированные или вовсе непросуммированные строки.

5. Что произойдёт, если не выполнить OPTIMIZE TABLE FINAL для ReplacingMergeTree?
**Если не выполнить OPTIMIZE TABLE FINAL для ReplacingMergeTree**, то старые версии строк (с меньшим значением версии) физически останутся в таблице и будут видны при обычном SELECT, и только со временем, при очередном фоновом слиянии партиций, они будут окончательно удалены.
